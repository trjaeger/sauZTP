...
def signString(keyFilePath, password, stringToSign, algo):
    ...
    #key = key_file.read()
    #key_file.close()
    key = getCertStringfromFile(keyFilePath)
...
devIDcertString = getCertStringfromFile(devID_cert)
#nonce = gen_nonce(50)
nonce = secrets.token_hex(32)

timestamp = datetime.now().isoformat()
#print(timestamp.strptime("30 Nov 00", "%d %b %y")=
#datetime_object = datetime.strptime(timestamp, 'HH[:MM[:SS[.fff[fff]]]][+HH:MM[:SS[.ffffff]]]')
...
sign = signString (client_key , b'password',byteRequest, "sha256" )
print(sign)
voucher_request = {
    "voucher-request" : {
    "request": byteRequest,
...
'''
import sys
print(sys.getsizeof(byte_voucher))
